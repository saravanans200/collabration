...
public class FriendDAOImpl implements FriendDAO{
	...
	public List<UserDetail> showSuggestedFriend(String username) {
		Session session=sessionFactory.openSession();
		Query query=session.createNativeQuery("select username from Userdetail where username not in (select friendname from Friend where username='"+username+"') and username not in (select username from Friend where friendname='"+username+"' and status='A') and username!='"+username+"'");
