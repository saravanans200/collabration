myApp.controller("JobController",function($scope,$location,$rootScope,$http)
{
	...
	$scope.addJob=function()
	{
		...
		$http.post('http://localhost:8090/CollMiddleware/addJob',$scope.job)
	...
	$scope.applyjob=function()
	{
		...
		$http.post('http://localhost:8090/CollMiddleware/applyjob',$scope.applyjob)
		.then(function(response)
		{
	...
	$scope.publishJob=function()
	{
		...
		$http.post('http://localhost:8090/CollMiddleware/publishJob',$scope.job)
		.then(function(response)
				{
					alert("Job published");
					$location.path("/showJob");
	...
	$scope.deleteJob=function(jobId)
	{
		...
		$http.delete('http://localhost:8090/CollMiddleware/deleteJob/'+jobId)
		.then(function(response)
				{
			listJobs();
			alert('job Deleted');
	...
	function listJobs()
	{
		...
		$http.get('http://localhost:8090/CollMiddleware/getJobs')
		.then(function(response)
				{
