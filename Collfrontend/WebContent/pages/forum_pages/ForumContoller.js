myApp.controller("ForumController",function($scope,$location,$rootScope,$http)
{
	...
	$scope.addForum=function()
	{
		...
		$http.post('http://localhost:8090/CollMiddleware/addForum',$scope.forum)
	...
	function listForums()
	{
		...
		$http.get('http://localhost:8090/CollMiddleware/showAllForum')
		.then(function(response)
				{
	...
	$scope.deleteForum=function(forumId)
	{
		...
		$http.delete('http://localhost:8090/CollMiddleware/deleteForum/'+forumId)
		.then(function(response)
				{
			listForums();
			alert('Forum Deleted');
	...
	$scope.approve=function(forumId)
	{
		...
		$http.get('http://localhost:8090/CollMiddleware/approveForum/'+forumId)
		.then(function(response)
				{
			listForums();
	...
	$scope.reject=function(forumId)
	{
		...
		$http.get('http://localhost:8090/CollMiddleware/rejectForum/'+forumId)
		.then(function(response)
				{
			listForums();
